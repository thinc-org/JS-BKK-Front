...
const useMockApi = (arg: string) => {
  let result: null | RestaurantGroup[] |Schedule[]= null;
  switch (arg) {
    case 'schedule':
      result = [
        {
        ...
        {
          key : 9,
          topics : "Talking about “Scale”: Takeaways from our attempt on scaling a small system in the Gojek Universe ",
        ...
        {
          key : 14,
          topics : "Applying SOLID principle in JavaScript without Class and Object ",
        ...
        {
          key : 17,
          topics : "Poor Man's Patcher: A game modder's adventure through serverless sea without money",
        ...
        {
          key : 21,
          topics : "[TBD] Speed up heavy data visualization with Rust and WebAssembly ",
      ...
      for (let i:number = 0; i <result.length-1;i+=1){
        ...
        result[i].happening = (
          ...
          compareAsc(
            new Date(),
            new Date(`2020-02-08T${result[i+1].hours}:${result[i+1].minutes}:00+07:00`)
          ) === -1
        );
      }      
      break;
  ...
  return useCallback(
    () =>
      new Promise<RestaurantGroup[] | null |Schedule[]>((resolve, reject) => {
